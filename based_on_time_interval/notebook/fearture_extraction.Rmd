...
```{python}
def adding_feature(rides_df: pd.DataFrame, number_intervals_per_day: int):
    ...
    rides_df['last_day_demand'] = rides_df.groupby(['PULocationID'])[
        'count'].shift(number_intervals_per_day)
    rides_df['last_week_demand'] = rides_df.groupby(['PULocationID'])[
        'count'].shift(number_intervals_per_day*7)

    return rides_df
```

```{python}
labels_time_df_feat = adding_feature(labels_time_df, 8)
