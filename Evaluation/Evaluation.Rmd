...
for key in predictions_dict:
    for locationID in PULocationIDs:
        ...
        metrics_all.append({'PULocationID': locationID, 'Model': key, 'MAPE': metric_mape,
                           'MAE': metric_mae, 'MSE': metric_mse, 'RMSE': metric_rmse})
...
def caculate_metrics(df, sorted_df, lower=0, upper=266):
    ...
    for key in predictions_dict:
        ...
        metric_mse = PULocationIDs_Metrics_Selected[PULocationIDs_Metrics_Selected['Model'] == key]['MSE'].mean(
        )
        metric_rmse = PULocationIDs_Metrics_Selected[PULocationIDs_Metrics_Selected['Model'] == key]['RMSE'].mean(
        )
        predictions_result.append({'Model': f"{key}_{lower}-{upper}", 'MAPE': metric_mape, 'MAE': metric_mae,
                                  'MSE': metric_mse, 'RMSE': metric_rmse})
...
```{python}
predictions_result = pd.DataFrame(caculate_metrics(
    xgb_rides_df_sorted, PULocationIDs_Metrics_sorted))
...
```{python}
predictions_result = pd.concat([predictions_result, pd.DataFrame(caculate_metrics(
    xgb_rides_df_sorted, PULocationIDs_Metrics_sorted, 0, 50))], ignore_index=True)
...
```{python}
predictions_result = pd.concat([predictions_result, pd.DataFrame(caculate_metrics(
    xgb_rides_df_sorted, PULocationIDs_Metrics_sorted, 50, 150))], ignore_index=True)
predictions_result = predictions_result.reset_index(drop=True)
```
...
```{python}
predictions_result = pd.concat([predictions_result, pd.DataFrame(caculate_metrics(
    xgb_rides_df_sorted, PULocationIDs_Metrics_sorted, 150, 250))], ignore_index=True)
predictions_result = predictions_result.reset_index(drop=True)
```

```{python}
print(predictions_result.shape)
predictions_result.tail()
```
